...
int main(int argc, char **argv)
{
	...
	int numbers;
	vector<int> vet_ints_sum;
	while(in_file>>numbers)
	{
		vet_ints_sum.push_back(numbers);
	}
	...
	cout<<"对文件中的数据求得的和:"<<accumulate(vet_ints_sum.begin(),
	vet_ints_sum.end(),0)<<endl;
	out_file<<"对文件中的数据求得的和:"<<accumulate(vet_ints_sum.begin(),
	vet_ints_sum.end(),0)<<endl;
	out_file.close();
	out_file.clear();
	return EXIT_SUCCESS;
