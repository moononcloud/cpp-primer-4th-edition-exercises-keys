...
int main()
{
    ...
    stable_sort(vet_str.begin(), vet_str.end(), isShorter());
    cout<<"请输入字符长度限定范围:"<<endl;
    size_t bound1,bound2;
    cin>>bound1>>bound2;    
    vector<string>::size_type wc=count_if(vet_str.begin(),vet_str.end(),GT_cls(bound1,bound2));
    cout<<wc<<" "<<make_plural(wc,"word","s")<<" "<<bound1<<"-"<<bound2<<" characters or longer"<<endl;
	return 0;
}
